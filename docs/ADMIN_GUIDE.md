...
`Lifecycle` `연동 · 알림` `AI 모델` `방문 추적` `메일 알림` `감사로그` `서버 로그` 입니다.
...
| mail | `mail.enabled` 외 14개 | `mail.enabled:false`, `mail.smtp_port:25`, `mail.security:auto` | 사내 SMTP 릴레이 메일 알림(3.6). 기본 꺼짐 |
...
**서비스 관리 → 메일 알림** 에서 사람이 실제로 기다리는 일 네 가지를 사내 SMTP 릴레이로
보냅니다. **기본은 꺼짐**이며, 새로 설치한 곳에서는 켜기 전까지 아무것도 나가지 않습니다.
설정 키 이름은 사내의 다른 서비스와 같아서(`mail.enabled`, `mail.smtp_host`, …) 한 번
배우면 어디서나 같습니다.
...
| `mail.smtp_port` | `25` | 사내 릴레이는 대개 25. 465 를 적으면 `auto` 에서 처음부터 TLS 로 붙습니다 |
| `mail.security` | `auto` | `auto` · `none` · `starttls` · `tls`. `auto` 는 서버가 STARTTLS 를 알리면 쓰고 아니면 평문 |
| `mail.skip_tls_verify` | `false` | 사내 인증서가 사설일 때만 |
| `mail.username` · `mail.password` | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택 사항**. 비우면 AUTH 를 시도하지 않습니다 |
| `mail.from_address` · `mail.from_name` | — · `Vendra` | 보내는 사람. 주소를 비우면 `vendra@<릴레이 주소>` |
| `mail.base_url` | — | 메일 속 「바로 열기」 링크가 가리킬 이 서비스의 주소. 비우면 링크 없이 보냅니다 |
| `mail.timeout_seconds` | `10` | 연결부터 전송 완료까지의 제한 시간 |
| `mail.notify_approval_request` | `true` | 결재 요청이 내 차례에 도착 |
| `mail.notify_approval_decision` | `true` | 상신한 건의 승인 · 반려 · 반송 |
| `mail.notify_expiry` | `true` | 계약 · 문서 만료 임박, 평가 기간 도래 |
| `mail.notify_alert` | `true` | SLA 위반 · 계약금액 초과 |

**보내는 이벤트와 고른 이유.** 기준은 하나 — 이 메일이 오지 않으면 누군가 손해를 보거나
화면을 계속 새로고침하는가.

1. **결재 요청이 내 차례에 도착** — 상신되거나 앞 단계가 승인되어 다음 단계로 넘어오면 그
   단계의 역할을 가진 사람 중 데이터 범위 안에서 그 건을 볼 수 있는 사람에게 갑니다. 이
   메일이 없으면 결재자가 승인함을 열어 볼 때까지 기안자가 기다립니다.
2. **상신한 건의 승인 · 반려 · 반송** — 기안자(와 담당자가 다르면 담당자)에게 갑니다.
   기안자가 새로고침하며 기다리는 바로 그 일입니다.
3. **계약 · 문서 만료 임박, 평가 기간 도래** — 놓치면 계약이 끊기고 문서가 효력을 잃습니다.
4. **SLA 위반 · 계약금액 초과** — 멈춰 있는 것, 즉시 손을 대야 하는 것.

단순한 「무언가 바뀜」은 보내지 않습니다. 그리고 **자기가 한 일은 자기에게 보내지
않습니다** — 결재자가 승인한 건의 결과가 결재자 본인에게 가지 않고, 상신한 사람이 자기
상신 요청을 받지 않습니다. 3·4 는 5.2 의 백그라운드 작업이 매시간 새로 찾은 것만, 사람마다
한 통으로 묶어 보냅니다. 열 건의 계약이 같은 시간에 만료 임박에 들어가면 한 통입니다.
한 번 보낸 것은 다시 보내지 않습니다.

**받는 사람.** 계정의 이메일입니다. 메일 기능은 따로 명부를 갖지 않고 사용자 표에서 id
를 주소로 바꾸는 조회만 씁니다. 비활성 계정에는 가지 않습니다.

**요청을 막지 않습니다.** 메일은 배경에서 보냅니다. 릴레이가 느리거나 죽어 있어도 상신과
승인은 평소처럼 200 으로 끝나고, 실패는 발송 기록에 남습니다. 연결이 거부되면 2초 뒤 한 번
더 시도합니다.

**비밀번호는 되읽히지 않습니다.** `mail.password` 는 `secret_value` 에 `ENCRYPTION_KEY` 로
암호화되어 저장되고, 설정 API 와 화면에는 「설정됨」만 보입니다. 비워 두고 저장하면 기존
값이 유지됩니다. 로그와 감사로그에도 값은 남지 않습니다.

**시험 발송.** 화면 아래 **시험 메일 보내기** 는 저장된 설정으로 실제 한 통을 보내고
릴레이의 대답을 그 자리에서 보여 줍니다. `mail.enabled` 가 꺼져 있어도 됩니다 — 모두에게
켜기 전에 릴레이를 증명하는 것이 이 단추의 목적입니다. 받는 사람을 비우면 내 주소로
보냅니다. `POST /api/v1/admin/mail/test` 가 같은 일을 하며, 릴레이가 답하지 않으면 502
`mail_send_failed` 와 이유가 옵니다.

**발송 기록.** 같은 화면의 **발송 기록**(`GET /api/v1/admin/mail/deliveries`)에 시도마다
남습니다 — 시각, 이벤트, 받는 사람, 제목, 결과(발송됨 · 실패 · 대기), 실패 이유. **본문은
담지 않습니다.** 본문까지 담으면 알림 기록이 그 자체로 반출 경로가 됩니다. 「안 왔다」는
문의에는 이 표로 답합니다.

**확인하는 법.** 릴레이 주소와 보내는 주소를 저장하고 시험 메일을 자기 주소로 보내 도착을
확인한 뒤 `mail.enabled` 를 켭니다. 이어 결재 규칙이 있는 업무 하나를 상신해 결재자에게
메일이 가고, 승인하면 기안자에게 가는 것을 발송 기록에서 봅니다. 릴레이를 끊어 놓고
상신해도 상신은 정상으로 끝나야 하며, 발송 기록에는 실패가 이유와 함께 남아야 합니다.
...
1. **알림 예약** — 기한이 다가오는 업무·만료가 가까운 계약과 문서. 메일 알림(3.6)이
   켜져 있으면 이 회차에 새로 찾은 것을 사람마다 한 통으로 묶어 보냅니다
2. **알림 발송** — 설정된 어댑터로 전달
...
5. **SMTP 릴레이**(3.6) — 켠 경우, 결재 요청·결과와 만료·조치 알림의 제목과 요약이
   릴레이로 갑니다. 릴레이를 사내(`postra`)로 두면 안에 머뭅니다. 발송 기록에는 본문이
   남지 않습니다.

다섯 다 서비스 관리에서 꺼 둘 수 있고, 꺼 두면 런타임은 외부와 통신하지 않습니다.
